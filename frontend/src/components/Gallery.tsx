...
export const Gallery = ({ level }: Props) => {
  ...
  const GalleryItems = () => {
    const listItem = achievedGallery.map((item) => (
      <div className="w-1/3" key={item}>
        <img src={item} alt="achievement"></img>
      </div>
    ));
    return <>{listItem}</>;
  };
  ...
  return (
    <div>
      ...
      <hr />
      {gallery == 9 ? (
        <h3 className="animate-pulse text-xs text-right mb-2 text-violet-500">
          Congratulations! You have completed the Swanky Magink! School <br /> Claim your Wizard NFT!
        </h3>
      ) : (
