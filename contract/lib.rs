...
pub mod magink {
  ...
  impl Magink {
    ...
    pub fn mint_wizard(&mut self) -> Result<String, PSP34Error> {
      ...
      match self.magink_nft.mint(self.env().caller()) {
        Ok(string) => {
          let mut profile = self.get_profile().unwrap();
          profile.badges_claimed += 1;
          profile.start_block = self.env().block_number();
          self.user.insert(self.env().caller(), &profile);
  ...
  mod tests {
    ...
    #[ink::test]
    fn start_works() {
      let mut magink = Magink::new(Hash::from([0x1; 32]), String::from("uri placeholder"));
